function returnProjectCard(projectName) {
  const projectCard = document.createElement("div");
  projectCard.classList.add("card", "project", projectName);
  ...
  title.textContent = projects[projectName].title;

  const subtitle = document.createElement("div");
  subtitle.classList.add("subtitle");
  subtitle.textContent = projects[projectName].subtitle;

  const intro = document.createElement("div");
  intro.classList.add("description");
  intro.textContent = projects[projectName].intro;

  projectInfo.appendChild(title);
  projectInfo.appendChild(subtitle);
  projectInfo.appendChild(intro);
  ...
  projects[projectName].tags.forEach((tag) => {
    const techItem = document.createElement("li");
    techItem.classList.add(tag);
  ...
  modalButton.innerHTML = openModalButton();

  cardBottom.appendChild(techs);
  cardBottom.appendChild(modalButton);
...
function formatTag(val) {
  return (
    String(val).charAt(0).toUpperCase() + String(val).slice(1).replace("-", " ")
  );
}
